...
const InvoicesTab: React.FC = () => {
  ...
  const handleUpdate = useCallback(
    (invoiceId: string, field: string, value: any) => {
      ...
      switch (field) {
        ...
        case 'totalAmount': {
          ...
          console.log('Updating customer total amount:', {
            customer,
            newValue: value,
          });
          if (customer) {
            ...
            const totalPurchaseAmount = customerInvoices.reduce(
              (sum, inv) =>
                sum +
                (inv.id === invoiceId ? Number(value) : inv.totalAmount || 0),
              0
            );
        ...
        case 'productName': {
          ...
          if (product) {
            ...
            const relatedInvoices = invoices.filter(
              (inv) => inv.productId === product.id && inv.id !== invoiceId
            );
        ...
        case 'customerName': {
          ...
          if (customer) {
            ...
            const relatedInvoices = invoices.filter(
              (inv) => inv.customerId === customer.id && inv.id !== invoiceId
            );
            console.log('Related invoices to update:', relatedInvoices);
            relatedInvoices.forEach((inv) => {
    ...
    () => [
      {
        ...
        render: (value: string, row: Invoice) => (
          <EditableCell
            value={value}
            onSave={(newValue) =>
              handleUpdate(row.id, 'serialNumber', newValue)
            }
          />
        ),
      },
      {
        ...
        render: (value: string, row: Invoice) => (
          <EditableCell
            value={value}
            onSave={(newValue) =>
              handleUpdate(row.id, 'customerName', newValue)
            }
          />
        ),
      },
      ...
      },
      {
        ...
        render: (value: number, row: Invoice) => (
          <EditableCell
            value={value}
            onSave={(newValue) =>
              handleUpdate(row.id, 'quantity', Number(newValue))
            }
            type="number"
          />
        ),
      ...
      },
      {
        ...
        render: (value: number, row: Invoice) => (
          <EditableCell
            value={value ?? 0}
            onSave={(newValue) =>
              handleUpdate(row.id, 'totalAmount', Number(newValue))
            }
            type="number"
          />
        ),
